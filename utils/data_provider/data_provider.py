...
def load_annoataion(p):
    ...
    #with open(p, 'r') as f:
    # reader = csv.reader(f)
    reader = file_lines_to_list(p)
    for line in reader:
        #label = line[-1]
        label = line.split(',', 8)[-1]
        # strip BOM. \ufeff for python3,  \xef\xbb\bf for python2
        #line = [i.strip('\ufeff').strip('\xef\xbb\xbf') for i in line]
        # Remove this
        #print("\n\nLabel is: {}".format(label))
        #print("\nLine is: {}".format(line))
        #print("Type of line is: {}".format(type(line)))
        #x1, y1, x2, y2, x3, y3, x4, y4 = list(map(float, line[:8]))
        try:
          x1, y1, x2, y2, x3, y3, x4, y4 = list(map(float, line.split(',', 8)[:8]))
        except ValueError:
          continue
